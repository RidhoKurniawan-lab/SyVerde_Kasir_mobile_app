...
class ProductNotifier extends StateNotifier<ProductState> {
  ...
  Future<void> insertProduct(ProductRequest request) async {
    final previousState = state; // simpan state sebelumnya, 
    state = ProductLoading();

    try {
      final product = await repository.insertProduct(request);
      if (previousState is ProductLoaded) {
        state = ProductLoaded([...previousState.products, product]); // pakai state sebelumya dan gabungkan dengan yang baru
      } else {
        await getProduct();
      }
    } catch (e) {
